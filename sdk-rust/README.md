...
### G. ⚠️ El `retry_after` del protocolo solo se cumple en el primer reintento

**Hallazgo nuevo, verificado contra nats-server 2.14.5 durante este port.** No está en la
spec y afecta a los seis SDKs por igual.
...
consumidor CON backoff, nak(300 ms):  entregas a 0, 300 ms, luego backoff[1] = 60 s
```

Consecuencia: un `Retry-After: 5` de un proveedor acorta el primer reintento y **nada más**;
del segundo en adelante se vuelve al backoff canónico. Es exactamente la misma trampa
familiar que `ack_wait ← backoff[0]` de [03-delivery.md §2.1](../specification/03-delivery.md):
el servidor acepta lo que le pides, no avisa, y hace otra cosa.

**No hay divergencia entre SDKs que arreglar** —los seis emiten el mismo `nak(delay)` y
obtienen el mismo comportamiento—, pero la spec y los README afirman más de lo que el
servidor cumple. **Sugerencia para la spec:** documentar en §2.1, junto a la trampa de
`ack_wait`, que `backoff` gobierna los reintentos a partir del segundo y que
`Classification.retryAfter` es por tanto un *hint del primer reintento*, no una garantía.
...
| `client.rs` | `connect`, `Bus::publish`, `Bus::subscribe`, `Bus::close`, `Bus::replay_from_dlq` |
...
| Suite | Tests | Requiere broker |
|---|--:|---|
| Unitarios (en cada módulo) | 87 | no |
| `tests/cross_sdk.rs` | 3 | no |
| `tests/integration.rs` | 7 | **sí** |
| Doctests | 8 | no |

Los unitarios cubren naming, envelope y clasificación, que es donde vive la semántica del
protocolo. `cross_sdk.rs` ejecuta **los fixtures compartidos de
...
cargo test --test integration    # 7 tests contra NATS real
```
...
ilegible **nunca** llegue al handler, y que las extensiones `dlq*` queden **antes** de `data`
en los bytes que acaban en la DLQ.
